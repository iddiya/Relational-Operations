       ...
       implements Serializable, Cloneable, Map <K, V>
{
    ...
    private void split() {
        ...
        // Retrieve the bucket chain to split
        Bucket splitBucket = hTable.get(isplit);

        // Create a new bucket chain for redistributed keys
        Bucket newBucket = new Bucket();

        // Set the new bucket chain as the next of the split bucket chain
        splitBucket.next = newBucket;

        // Increment the split index
        isplit++;

        // Iterate through the keys in the split bucket chain
        for (int i = 0; i < splitBucket.nKeys; i++) {
            K key = splitBucket.key[i];
            V value = splitBucket.value[i];

            // Compute the high-resolution hash value for the key
            int hash = h2(key);
        ...
        // If the current split phase is complete, reset 'isplit' to zero and update the hash functions
        if (isplit == mod1) {
            isplit = 0;
            mod1 *= 2;
            mod2 = 2 * mod1;
